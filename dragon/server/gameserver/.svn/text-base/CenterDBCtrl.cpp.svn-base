...
bool CenterDBCtrl::BackupPassportOfRole( DatabaseMysql* dstDB, uint32 dwRoleId )
{
	...
	if ( !result )
	{
		IME_SQLERROR( "Error Backup Passport To GameServer, passportId=%llu", (unsigned long long)dwPassportId );
		return false;
	}
	field = result->Fetch();
...
//		}
	...
	else
	{
		...
		return m_db->PExecute( "insert into player_info( role_id, name, gm_auth, status, progress, level, "
				"gold, diamond, cur_stage, cur_train, vip_level, vip_exp, stamina, energy) values( %u, '%s', %u, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u)",
...
bool CenterDBCtrl::UpdateRoleForbidAuth(
		uint32			dwRoleId,
		uint8			byForbidAuth )
{
	return m_db->PExecute("update player_info set status=%u where role_id=%u", byForbidAuth, dwRoleId );
}
