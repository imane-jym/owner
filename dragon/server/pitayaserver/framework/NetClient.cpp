...
//	:evbase(_evbase)
:m_pSession(NULL),m_bIsClose(false),m_pServer(pServer),m_sockFd(nFd)
{
...
void CNetClient::SendSinglePacket()
{
    ...
    //if (!m_sendQueue.empty())
    {
        ...
        hdr.size = (uint16)packet->size() + 4;//ntohs((uint16)packet->size() + 2);
        hdr.cmd = packet->GetOpcode();
        ...
        char buffer[1024];
        int len = 0;
        memcpy(buffer, (char *)&hdr, 4);
        memcpy(buffer + 4, (char *)packet->contents(), packet->size() > 800 ? len = 800 : len = packet->size());
        len += 4;
        hexdump(buffer, len);

        ///- Send the header and body to the client
        m_pBufferOutput->Append((char*)&hdr, 4);
        if(!packet->empty()) 
        {
