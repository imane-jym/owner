...
void CHandlerPlayersession::HandlerClientReg(WorldPacket & packet)
{
	...
	{
		SendRegResult(ERRNO_MSG_AUTH_NAME_LEN,strAccount,strPwd);
		return ;
	}
	if (strPwd.size() == 0 || strPwd.size() > E_MAX_USERNAME_PWD_LEN)
	{
		SendRegResult(ERRNO_MSG_AUTH_PWD_LEN,strAccount,strPwd);
		return ;
	}
	if (strAccount.size() > 2 && strAccount.at(0) == 'u' && CUtil::IsDigit(strAccount.at(1)))
	{
		SendRegResult(ERRNO_MSG_AUTH_ACCOUNT_HAVE,strAccount,strPwd);
		return ;
	}
//	if (!(wChannel == E_CHANNEL_IOS || wChannel == E_CHANNEL_ANDROID || wChannel == E_CHANNEL_WIN))
//	{
//		SendRegResult(ERRNO_MSG_AUTH_CHANNEL, strAccount, strPwd);
//		return;
//	}
...
//	LoginDatabase.escape_string(strUid);
	...
	LoginDatabase.EndCommit();
	SendRegResult(ERRNO_MSG_AUTH_REG_SUCCESS,strAccount,strPwd);
}
...
//	{
//		SendLoginResult(ERRNO_MSG_AUTH_WRONG_VERSION);
//		return ;
//	}
	...
	if (strAccount.size() == 0 || strAccount.size() > E_MAX_USERNAME_PWD_LEN)
	{
		SendLoginResult(ERRNO_MSG_AUTH_NAME_LEN);
		return ;
	}
	if (strPwd.size() == 0 || strPwd.size() > E_MAX_USERNAME_PWD_LEN)
	{
		SendLoginResult(ERRNO_MSG_AUTH_PWD_LEN);
		return ;
	}
//	if (!(wChannel == E_CHANNEL_IOS || wChannel == E_CHANNEL_ANDROID || wChannel == E_CHANNEL_WIN))
//	{
//		SendLoginResult(ERRNO_MSG_AUTH_CHANNEL);
//		return ;
//	}
...
//		accountId = fields[0].GetUInt32();
		...
		if (nIsAuto && strPwdReal.length() == 0)
		{
			//自动生成的帐号 并且没有修改过密码
			{
				SendLoginResult(ERRNO_MSG_AUTH_NO_PWD);
				return ;
			}
		}
		else
...
//				{
					SendLoginResult(ERRNO_MSG_SUCC);
//				}
//				else
//				{
//					SendLoginResult(ERRNO_MSG_AUTH_NO_SERVER);
//				}
			}
			else
			{
				//密码不正确
				SendLoginResult(ERRNO_MSG_AUTH_WRONG_PWD);
			}
		}
	}
	else
	{
		//没有该用户
		SendLoginResult(ERRNO_MSG_AUTH_NO_ACCOUNT);
	}
}
...
//	if (dwVersion != INNER_VERSION)
//	{
//		SendFastLoginResult(ERRNO_MSG_AUTH_WRONG_VERSION);
//		return ;
//	}
//	if (!(wChannel == E_CHANNEL_IOS || wChannel == E_CHANNEL_ANDROID || wChannel == E_CHANNEL_WIN))
//	{
//		SendFastLoginResult(ERRNO_MSG_AUTH_CHANNEL);
//		return ;
//	}
	LoginDatabase.escape_string(strUid);
...
//		{
//			SendFastLoginResult(ERRNO_MSG_SUCC,strIp,nPort);
//		}
//		else
//		{
//			SendFastLoginResult(ERRNO_MSG_AUTH_NO_SERVER,strIp,nPort);
//		}
		SendFastLoginResult(ERRNO_MSG_SUCC);
		delete result;
		return ;
	}
	else
	{
		...
		SendFastLoginResult(ERRNO_MSG_AUTH_SYS);
...
void CHandlerPlayersession::CharacterCreate(uint64 account_id, uint32 &character_id, uint16 serverid)
{
	...
	else
	{
		...
		character_id++;
		character_id = character_id << 8 | character_id % 8;
...
//	if (dwVersion != INNER_VERSION)
//	{
//		SendOtherLoginResult(ERRNO_MSG_AUTH_WRONG_VERSION);
//		return ;
//	}
//	if (!(wChannel == E_CHANNEL_IOS || wChannel == E_CHANNEL_ANDROID || wChannel == E_CHANNEL_WIN))
//	{
//		SendOtherLoginResult(ERRNO_MSG_AUTH_CHANNEL);
//		return ;
//	}
	...
	if (it == mapTemp.end())
	{
		...
		SendOtherLoginResult(ERRNO_MSG_AUTH_NO_SERVER);
		return;
	}
...
//		if (ChooseServer(dwVersion,strIp,nPort,dwAccountId))
//		{
//			Send91LoginResult(ERRNO_MSG_SUCC,strIp,nPort);
//		}
//		else
//		{
//			Send91LoginResult(ERRNO_MSG_AUTH_NO_SERVER,strIp,nPort);
//		}
				...
				if (characterid == 0)
				{
					...
					SendOtherLoginResult(ERRNO_MSG_AUTH_SYS);
					delete result;
					return;
				}
...
//			IME_ERROR("flag %u", m_byFlag);
			...
			SendOtherLoginResult(ERRNO_MSG_SUCC);
		}
		else
		{
			SendOtherLoginResult(ERRNO_MSG_AUTH_UNVALID);
		}
		delete result;
		return ;
	}
...
void CHandlerPlayersession::HandlerAd(WorldPacket &pkg)
{
	...
	for (unsigned int i = 0; i < g_vecAd.size(); i++)
	{
		info << g_vecAd[i];
	}
	SendPacket(&info);
