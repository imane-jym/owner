...
bool CConfUpgrade::ReadConfig(const std::string& strFilePath, MapData &map)
{
	...
	for (int i = 1; i < nRow; ++i)
	{
		...
		info.dwCombineId = GET_UPGRADE_COMBINE_ID(info.wLevel, info.dwIndex);
...
//	uint32_t olditem = 0;
	...
	IME_LOG("LOAD CONF AREA [Size=%lu]", map.size());
	return true;
}
...
std::vector<const STC_CONF_AREA*> CConfArea::FindbyAreaId( uint32 dwAreaId )
{
	MapData::const_iterator it = m_mapData.begin();

	std::vector<const STC_CONF_AREA*> v;

	while ( it != m_mapData.end() )
	{
		if ( it->second.wArea == dwAreaId )
		{
